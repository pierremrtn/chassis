...
Consider how this works in practice. A Handler that creates users depends on an `UserRepository` interface:
...
This distinction enables the Mediator to route requests to the appropriate handler type. Attempting to `watch` a ReadQuery — or to `read` a WatchQuery — is a compile error: each dispatch method accepts only its own query subtype.
...
class UserViewModel extends ViewModel<UserState, UserEvent> {
  ...
  void createUser(String name, String email) {
    run(
      () => _mediator.run(CreateUserCommand(name: name, email: email)),
...
Handler registration happens at application startup, creating a type-safe registry. Wiring mistakes surface immediately and unambiguously: registering two handlers for the same message type throws a `DuplicateHandlerException`, and dispatching a message that has no registered handler throws a `HandlerNotRegisteredException` with an actionable message. Both extend the sealed `ChassisException` type and behave identically in debug and release builds — they signal programming errors to fix, not runtime conditions to catch.

### Type Safety and Discoverability

The mediator generated by Chassis provides compile-time type safety.
...
```dart
// Generated by chassis_builder from @ChassisApp: one typed method per handler,
// each named after its message and mirroring the message constructor's
// parameters, dispatching through the mediator so middleware always applies.
class AppMediator extends Mediator {
  // ...constructor registers every handler with its dependencies...

  // Commands (writes) - dispatch through run()
  ...
  // Queries (reads) - dispatch through read()
  Future<User> getUser({required String userId}) =>
      read(GetUserQuery(userId: userId));

  // Queries (reactive reads) - dispatch through watch()
  Stream<List<User>> watchUsers() => watch(WatchUsersQuery());
}

// IDE autocomplete shows all available operations with type safety
await mediator.createUser(name: 'John', email: 'john@example.com');
final user = await mediator.getUser(userId: 'user-123');
mediator.watchUsers().listen((users) => print(users));
```

The typed methods are instance methods of the generated mediator class, produced from your `@chassisHandler` annotations — see [Code Generation](03_code_generation.md) for how the class is composed and named.
...
Chassis ships a ready-made `LoggingMiddleware` that traces every operation — dispatch, outcome, duration, and errors with stack traces:
...
Each trace renders the message's `params` (`CreateUserCommand{name: John}` instead of a bare type name), so override `params` on your commands and queries to make logs useful — and never include secrets in them. Records can be routed to any logger by providing a custom `ChassisLogSink`.

For custom concerns, extend `MediatorMiddleware` and override the hooks you need — `onRun` for commands, `onRead` for one-shot queries, `onWatch` for streaming queries. Each has a pass-through default:
